...
def combine_batches(files, histories):
    ...
    if len(files)!=len(histories):                                                                    #test case, each file must have a supplied history
        print("\nNot all simulation had their history count specified!\n")
        sys.exit()                                                                   
                
        initial_value, initial_stddev, header, coords = read_data(files[0], histories[0])             #initialize values from first file   
        hist_0 = histories[0]
        
        for i in range(1,len(files)):                                                                 #loop until all files have been merged, excludes first file  
            
            additional_value, additional_stddev = read_data(files[i], histories[i])[:2]               #read data from next file
            hist_1 = histories[i]
            new_value = [ (hist_0*initial_value[j]+hist_1*additional_value[j])/(hist_0+hist_1)\
                        for j in range(len(initial_value))]
            new_stddev = [ ( (( (hist_0 - 1)*initial_stddev[j] + (hist_1 - 1)*additional_stddev[j] )\
                              /(hist_0 + hist_1 - 1) ) \
            ...
            initial_stddev = new_stddev                                                               # number of histories as initial values for next calculation
            hist_0 += hist_1                                                                           
...
def create_new_file(filename, value, stddev, header, coords):
    ...
    datalines = [str(coords[i][0]) + ", " + str(coords[i][1]) + ", " + str(coords[i][2]) +\
                 ", " + str(value[i]) + ", " +str(stddev[i])  + "\n" for i in range(len(value))]
